...
public class UI_UpsertActionsForCheckID extends VoltProcedure {
	...
	public final SQLStmt updateAction = new SQLStmt(
			" UPDATE ACTION SET ACTIONNAME = ?, TARGETENDPOINTID = ?, CHECKID =?, CHECKPATHID = ? WHERE ACTIONID = ?");
	...
	public VoltTable[] run(String actionID, String actionName,
			String endpointID, String checkID, String checkpathID,
			String propertyID, String propertyIdIndex, String manualValue)
			throws VoltAbortException {

		voltQueueSQL(updateAction, actionName, endpointID, checkID,
		...
		if (rowsAffected1 == 0) {
			voltQueueSQL(addAction, actionID, actionName, endpointID, checkID,
					checkpathID);
			voltExecuteSQL();
		}
